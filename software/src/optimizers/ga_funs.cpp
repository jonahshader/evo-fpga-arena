...
Solution tournament_select_single(const Population &evaled_pop, size_t tournament_size,
                                  std::mt19937 &rng) {
...
PriorBestSelect make_tournament_prior_best(size_t size) {
  return [=](const Population &evaled_pop, std::mt19937 &rng) {
...
Fitness make_game_fitness_2p(std::shared_ptr<Game> game) {
  ...
  return [=](Solution &sol, std::vector<std::shared_ptr<Model>> &refs,
             std::vector<std::shared_ptr<Model>> &prior_best, const std::vector<uint64_t> &seeds) {
    ...
    for (auto &opponent : prior_best) {
      for (auto seed : seeds) {
        game->init(seed);
        std::vector<std::shared_ptr<model::SimpleModel>> models;
